...
export default function StatisticsPage() {
  ...
  useEffect(() => {
    async function loadStats() {
      try {
        ...
        if (!response.ok) {
          const responseBody = await response.json().catch(() => null) as StatsError | null;
          throw new Error(responseBody?.code === "outside-campaign-window"
            ? "Die oeffentliche Statistik wird waehrend des Weltrekordversuchs angezeigt."
            : "Die Statistik ist gerade nicht verfuegbar.");
        }
  ...
  return (
    <main className="stats-page">
      ...
      <section className="stats-hero"><p className="brand-kicker">Live-Auswertung</p><h1 className="sticker-head is-mint"><span className="sticker">Reparaturen</span><span className="sticker">sichtbar machen</span></h1><p>Die Zahlen zeigen ausschliesslich freigegebene Einreichungen.</p></section>
