...
- [x] Spam-Schutz für die Produktionsdomain vorbereitet (Issue #59): Widget-Fehler nennen jetzt ihren Code (`sitekey_invalid` statt "bitte erneut versuchen"), ein falscher API-Key gilt als Störung und nicht als Spam, ein aktiver Bypass steht im Systemstatus als `Gestoert`, und Limit sowie Captcha-Absagen landen im Einreichungsprotokoll. Offen bleiben die Schritte im Friendly-Captcha-Dashboard und in Vercel: Domain `reparatur.fab-bergisch.org` freigeben, `FRIENDLY_CAPTCHA_API_KEY` setzen, `NEXT_PUBLIC_CAPTCHA_ENABLED=true` und neu deployen (die öffentlichen Variablen werden beim Build eingebacken).
- [x] Einreichungslimit auf Reparatur-Café-Betrieb umgestellt (Issue #59): 40 Einreichungen je Minute und Verbindung statt 30 je fünf Minuten. Die Rate ist höher und die Wartezeit nach einem Treffer kürzer; greift das Limit, steht ein Eintrag im Einreichungsprotokoll statt nichts.
- [ ] Friendly-Captcha-Kontingent klären: Der kostenlose Plan deckt 1 Domain und 1.000 Anfragen im Monat ab, nicht-kommerziell - für einen Rekordversuch zu wenig, denn jeder Formularaufruf zählt. Für Open-Source-Projekte gibt es unter <https://friendlycaptcha.com/for-open-source> denselben Umfang wie im Advanced-Plan (50 Domains, 50.000 Anfragen im Monat) kostenlos; dieses Repository ist öffentlich und BSD-2-Clause. Mit Advanced-Zugang wäre auch der EU-Endpunkt (`eu.frcapi.com`) nutzbar - das sind zwei Stellen im Code, das Widget (`apiEndpoint`) und die Siteverify-URL.
- [ ] Geprüfte Alternative, falls der Drittanbieter ganz wegfallen soll: ALTCHA (MIT, `altcha` + `altcha-lib` mit offiziellem Next.js-Plugin). Vollständig selbst gehostet, kein CDN, kein Cookie, keine Anfrage nach außen - Friendly Captcha fiele damit aus Datenschutzerklärung und Auftragsverarbeitung heraus. Kehrseite: Proof-of-Work kostet einen Bot Rechenzeit, erkennt ihn aber nicht, während Friendly Captcha v2 zusätzlich Verhaltensanalyse macht. Zu beachten wäre der Replay-Schutz: ALTCHAs eingebauter Speicher liegt im Arbeitsspeicher der Instanz und gehört auf Vercel nach Postgres, analog zu `submission_throttle`.