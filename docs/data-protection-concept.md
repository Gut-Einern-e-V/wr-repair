...
| Gewinnspiel | Name und E-Mail-Adresse, aber nur bei ausdruecklicher Anmeldung ueber das Haekchen im Formular | Tabelle `lottery_entries`, getrennt von den Reparaturangaben und nur mit dem Service-Role-Key lesbar. Sie werden nie veroeffentlicht und erscheinen in keiner oeffentlichen Route. Dazu kommen der gewonnene Preis (`prize_id`), der Zeitpunkt der Ziehung und ein moeglicher Ausschluss (`excluded_at`). |
| Ausschlussliste der Verlosung | E-Mail-Adressen und Domains, die nicht gewinnen koennen - das Projektteam und die Durchfuehrenden | Tabelle `lottery_exclusions`, nur Superadmins. Sie enthaelt bewusst keine Namen und keinen Grund ueber die freiwillige Notiz hinaus. |
...
- Die Verlosung liegt vollstaendig bei den Superadmins: Preise, Ausschlussliste, Ziehung und der CSV-Export der gezogenen Personen (`/api/admin/lottery/export`). Dieser Export enthaelt Namen und E-Mail-Adressen und ist damit die sensibelste Datei des Projekts; er wird nicht zwischengespeichert (`Cache-Control: no-store`).
- Die Buehnenziehung unter `/tombola` zeigt Name, Kreis, Gegenstand und Reparaturgeschichte der gezogenen Person, aber ausdruecklich **nicht** ihre E-Mail-Adresse: Die Seite laeuft auf einer Leinwand vor Publikum. Die Liste aller Teilnehmenden verlaesst den Server auch fuer die Animation nicht - was dort durchlaeuft, sind die bereits gezogenen Namen.
