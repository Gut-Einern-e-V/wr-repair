...
A hard rejection is not the end of the road, though (issue #107). The hidden `frc-captcha-response` field also carries the widget's *state* — `.SOLVING` while it computes, `.EXPIRED` once the solution is too old — so the field is never empty and its mere presence proves nothing. Two things follow, and both are implemented: the browser waits for a real solution instead of submitting a state word (`.SOLVING` reached siteverify as `response_invalid` and cost the submission), and every captcha rejection answers with `captchaStale: true` so the form fetches a fresh solution. Without that second half the next press resubmitted the spent solution, siteverify answered `response_duplicate`, and the submitter was stuck in a loop that only a page reload — losing every entry — could break.
